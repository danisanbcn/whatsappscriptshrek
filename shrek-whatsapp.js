...
THE END
`;
const LINE_DELAY_MS = 700;   // pausa entre líneas (sube si te ratea: 900-1500)
const LONG_LINE_LIMIT = 1500; // si una línea supera esto, se trocea en segmentos
const SKIP_EMPTY_LINES = false; // true = no manda líneas vacías
...
async function sendOne(text) {
  ...
  if (!box) throw new Error('No encontré el cuadro de texto. Abre un chat y reintenta.');
  setText(box, text);
  await sleep(60);
  if (!clickSendButton()) pressEnter(box);
  await sleep(120);
}
...
// === MAIN: línea por línea ===
(async () => {
  const lines = RAW_TEXT.replace(/\r\n/g, '\n').split('\n');

  // construimos la cola de mensajes, troceando solo las líneas largas
  const queue = [];
  for (const line of lines) {
    const trimmed = line; // no recortamos: respetamos espacios originales
    if (SKIP_EMPTY_LINES && trimmed.trim() === '') continue;
    if (trimmed.length > LONG_LINE_LIMIT) {
      queue.push(...splitLongLine(trimmed, LONG_LINE_LIMIT));
    } else {
      queue.push(trimmed);
    }
  }

  console.log(`Enviando ${queue.length} líneas...`);
  for (let i = 0; i < queue.length; i++) {
    try {
      await sendOne(queue[i] === '' ? ' ' : queue[i]); // WhatsApp no siempre manda vacío, usamos espacio
      console.log(`✔️ Línea ${i + 1}/${queue.length}`);
      await sleep(LINE_DELAY_MS);
    } catch (e) {
      console.warn(`⚠️ Fallo en la línea ${i + 1}: ${e.message}. Reintentando en 1.5s...`);
      await sleep(1500);
      await sendOne(queue[i] === '' ? ' ' : queue[i]);
      await sleep(LINE_DELAY_MS);
    }
  }
  console.log('🎉 Terminado (línea por línea).');
})();
